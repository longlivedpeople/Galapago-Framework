...
class Efficiency:
   ...
   def changeLabelsToNames(self):
      ...
      for il,lab in enumerate(self.labels):
         print 'changing label %s to %s'%(lab, self.effs[il].GetName())
         newlabels.append(self.effs[il].GetName())
   ...
   def banner(self):
      ...
      latexb.DrawLatex(0.40, 0.93, "#it{Internal}")
   ...
   def addRate(self, eff, option, label, labelOption, color, ToDraw, orderForLegend, doOF = False):
      ...
      #self.effs.append(eff if not doOF else self.makeOFHisto(h))
      self.effs.append(_eff)
   ...
   def makeLegend(self):
      ...
      for i in range(0, len(self.effs)):
          for j in range(0, len(self.orderForLegend)):
              if(self.orderForLegend[j] != -1 and self.orderForLegend[j] == i):
                  self.myLegend.AddEntry(self.effs[j], self.labels[j], self.labelsOption[j])
   ...
   def saveRatio(self, legend, isData, log, lumi, hdata, hMC, r_ymin=0, r_ymax=2, label ="Data/Prediction", outputDir = 'plots/'):
      ...
      for i in range(0, len(self.effs)):
          if(self.ToDraw[i] != 0):
              #self.effs[i].SetMinimum(0.00001)
              self.effs[i].SetMinimum(0.1)
              #self.effs[i].SetMinimum(0.0001)
              #self.effs[i].SetMaximum(0.5)
              self.effs[i].Draw(self.options[i])
   ...
   def save(self, legend, isData, log, labelx, ymin=0, ymax=0, outputDir = 'plots/', xlog = False):
