...
#    make2DPlot(queue = opts.qenv, lumi = lumi, var = 'fabs(EEBase_dPhi[EEBase_maxIxy]):EEBase_trackIxy[EEBase_maxIxy]', name = '2D_350_148_173', nbinx = 15, xmin =  0, xmax = 60, nbiny = 6, ymin = 0, ymax = 3.14, xlabel='EE I_{xy}', ylabel = 'Collinearity |#Delta#Phi|', tree = treeSI, cuts = cuts.AddListB([cuts.EEChannel, cuts.haveEEBase]), outtag = opts.out)
    ...
    #makePlot(opts.qenv, lumi, 'nPUTrue', 'nPUTrue', 28, 0, 70, 'Number of true primary vertices', True, treeMC, cuts.AddListB([cuts.nTrack, '(nPUTrue < 20 || nPUTrue > 35) && (nEEBase > 0 || nMMBase >0)']), treeSI)

    
    #### Number of candidates
    #makePlot(opts.qenv, lumi, 'nElectronCandidate', 'nElectronCandidate', 7, 0, 7, 'Number of reconstructed electrons', True, treeMC, cuts.AddListB([cuts.nTrack, cuts.EEChannel]), treeSI)
    ...
    #makePlot(opts.qenv, lumi, 'fabs(IsoTrackSel_dxy[ElectronCandidate_isotrackIdx]/IsoTrackSel_dxyError[ElectronCandidate_isotrackIdx])', 'EC_dxy', 30, 0, 20.0, 'Transverse impact parameter |d_{xy}|/#sigma', True, treeMC, cuts.AddListB([cuts.nTrack, cuts.EEChannel]), treeSI)
