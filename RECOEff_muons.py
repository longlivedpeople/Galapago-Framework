...
if __name__ == "__main__":
    ...
    dxy_bin = np.linspace(0.0, 60.0, 51)
    ...
    ####   Book TFakes objects   ####
    #################################
    fake_IFCA_pt = r.TEfficiency("fake_IFCA_pt", ";Reconstructed #mu p_{T} (GeV);Fake Rate", len(pt_bin)-1, pt_bin)
    fake_IFCA_dxy = r.TEfficiency("fake_IFCA_dxy", ";Reconstructed #mu d_{xy} (cm);Fake Rate", len(dxy_bin)-1, dxy_bin)

    fake_SA_pt = r.TEfficiency("fake_SA_pt", ";Reconstructed #mu p_{T} (GeV);Fake Rate", len(pt_bin)-1, pt_bin)
    fake_SA_dxy = r.TEfficiency("fake_SA_dxy", ";Reconstructed #mu d_{xy} (cm);Fake Rate", len(dxy_bin)-1, dxy_bin)

    fake_DSA_pt = r.TEfficiency("fake_DSA_pt", ";Reconstructed #mu p_{T} (GeV);Fake Rate", len(pt_bin)-1, pt_bin)
    fake_DSA_dxy = r.TEfficiency("fake_DSA_dxy", ";Reconstructed #mu d_{xy} (cm);Fake Rate", len(dxy_bin)-1, dxy_bin)

    fake_RSA_pt = r.TEfficiency("fake_RSA_pt", ";Reconstructed #mu p_{T} (GeV);Fake Rate", len(pt_bin)-1, pt_bin)
    fake_RSA_dxy = r.TEfficiency("fake_RSA_dxy", ";Reconstructed #mu d_{xy} (cm);Fake Rate", len(dxy_bin)-1, dxy_bin)

    fake_DGM_pt = r.TEfficiency("fake_DGM_pt", ";Reconstructed #mu p_{T} (GeV);Fake Rate", len(pt_bin)-1, pt_bin)
    fake_DGM_dxy = r.TEfficiency("fake_DGM_dxy", ";Reconstructed #mu d_{xy} (cm);Fake Rate", len(dxy_bin)-1, dxy_bin)
    ...
    for i in range(0, _tree.GetEntries()):
        ...
        if True:
            ...
            for j in range(0, _tree.nGenLepton):
                ...
                if _tree.GenLeptonSel_pt[j] < 31: continue
            ...
            deltaR = 9999.0
            index = -9
            for j in range(0, _tree.nDSA):
                if abs(_tree.DSA_eta[j]) > 2: continue
                if _tree.DSA_pt[j] < 31: continue
                ...
                if deltaR < MAX_DELTAR:
                    fake_DSA_pt.Fill(False, pt)
                    fake_DSA_dxy.Fill(False, dxy)
                else:
                    fake_DSA_pt.Fill(True, pt)
                    fake_DSA_dxy.Fill(True, dxy)
            ...
            for j in range(0, _tree.nSA):
                ...
                if deltaR < MAX_DELTAR:
                    ...
                    fake_SA_dxy.Fill(False, dxy)
                else:
                    fake_SA_pt.Fill(True, pt)
                    fake_SA_dxy.Fill(True, dxy)
            ...
            for j in range(0, _tree.nDGM):
                ...
                if deltaR < 0.1:
                    fake_DGM_pt.Fill(False, pt)
                    fake_DGM_dxy.Fill(False, dxy)
                else:
                    fake_DGM_pt.Fill(True, pt)
                    fake_DGM_dxy.Fill(True, dxy)
            ...
            deltaR = 9999.0
            index = -9
            for j in range(0, _tree.nMuonCandidate):
                if abs(_tree.MuonCandidate_eta[j]) > 2: continue
                if _tree.MuonCandidate_pt[j] < 31: continue

                pt = _tree.MuonCandidate_pt[j]
                eta = _tree.MuonCandidate_eta[j]
                phi = _tree.MuonCandidate_phi[j]
                dxy = _tree.MuonCandidate_dxy[j]
    ...
    print("SA global eff: ", eff_SA_Lxy.GetTotalHistogram().GetEntries())
...
#    HIST_ptRes.addHisto(hist_GM_ptRes, 'HIST, SAME', 'GM', 'l', r.kAzure-4, 1, 1)
    HIST_ptRes.addHisto(hist_DGM_ptRes, 'HIST, SAME', 'DGM', 'l', r.kAzure-3, 1, 1)
    HIST_ptRes.addHisto(hist_SA_ptRes, 'HIST, SAME', 'SA', 'l', r.kRed-7, 1, 2)
#    HIST_ptRes.addHisto(hist_RSA_ptRes, 'HIST, SAME', 'RSA', 'l', r.kRed-4, 1, 4)
    HIST_ptRes.addHisto(hist_DSA_ptRes, 'HIST, SAME', 'DSA', 'l', r.kRed+1, 1, 4)
