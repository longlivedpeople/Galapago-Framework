...
class modelSet:
    ...
    def addModel(self, eleFile, muFile, mH, mX, ctau):

        self.eleFiles.append(eleFile)
        self.muFiles.append(muFile)
        self.mHs.append(mH)
        self.mXs.append(mX)
        self.ctaus.append(ctau)
        self.number+=1
...
if __name__ == "__main__":
    ...
    parser.add_option('-r', '--rebin', action='store', type=int, dest='rebin', default=0, help='rebinning')
    ...
    #models = modelSet()
    #models.addModel('efficiencies_400_50_400_ele/ElectronEfficiencies.root', 'MuonEfficiencies.root', 400, 50, 400)


    _file = TFile(opts.file)
        
    _h = _file.Get(opts.histo)
    #_h.Scale(1.0/_h.Integral())
    if opts.rebin: _h.Rebin(opts.rebin)
    _h.SetMarkerStyle(20)
    _h.SetMarkerColor(r.kBlack)
    _h.SetLineColor(r.kBlack)
    _h.SetLineWidth(2)
    _h.GetYaxis().SetTitle('Pairs')


    ## Define the gaussian function:
    f1 = r.TF1("f1","gaus",-0.5,0.5)        
    ...
    FIT.addLatex(0.2, 0.85, 'H#rightarrowXX#rightarrow4l', size = 0.03)
    FIT.addLatex(0.2, 0.81, 'm_{H} = '+str(opts.m1)+str(' GeV, m_{X} = ' + str(opts.m2) + str(' GeV, c#tau = ') + str(opts.ctau) + str(' mm')), size = 0.03)
    #FIT.addLatex(0.2, 0.76, 'H#rightarrowXX#rightarrow4l', size = 0.03)
    #FIT.addLatex(0.2, 0.71, 'm_{H} = '+str(opts.m1)+str(' GeV'), size = 0.03)
    #FIT.addLatex(0.2, 0.67, 'm_{X} = '+str(opts.m2)+str(' GeV'), size = 0.03)
    #FIT.addLatex(0.2, 0.63, 'c#tau  = '+str(opts.ctau)+str(' mm'), size = 0.03)
    #FIT.addGraph(_eh, 'hist', '', 'AP', r.kBlack, 1, 0)
    FIT.save(0, 0, 0, '', '', outputDir = 'Fits/', maxYnumbers = 2)
