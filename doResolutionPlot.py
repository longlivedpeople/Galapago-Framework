...
if __name__ == "__main__":
    ...
    models.append([1000, 150, 100, 0.02262, 0.00006])
    ...
    ############# Sigma plot declaration
    histo = r.TH1F('histo', '', len(models), 0, len(models))
    histo.GetXaxis().SetLabelSize(0)
    histo.GetYaxis().SetTitle("Fitted #sigma")
    histo.GetXaxis().SetNdivisions(len(models))
    histo.SetFillColorAlpha(r.kAzure-4, 0.4)
    histo.SetLineColor(r.kAzure-4)
    histo.SetLineWidth(2)
    #histo.SetFillStyle(3009)

    for _m,model in enumerate(models):

        histo.SetBinContent(_m + 1, model[3])
        histo.SetBinError(_m + 1, model[4])


    histo.SetMinimum(0.6*histo.GetMinimum())
    ...
    # labels:
    SIGMAS.addLatex(0.01, ctau_y, 'c#tau (mm):', size = 0.03, align = 12)
    SIGMAS.addLatex(0.01, mX_y, 'M_{X} (GeV):', size = 0.03, align = 12)
    SIGMAS.addLatex(0.01, mH_y, 'M_{H} (GeV):', size = 0.03, align = 12)
    for _m,model in enumerate(models):
        SIGMAS.addLatex(leftMargin + (_m + 0.5)*binWidth, ctau_y, str(model[2]), size = 0.03, align = 22)
        SIGMAS.addLatex(leftMargin + (_m + 0.5)*binWidth, mX_y, str(model[1]), size = 0.03, align = 22)
        SIGMAS.addLatex(leftMargin + (_m + 0.5)*binWidth, mH_y, str(model[0]), size = 0.03, align = 22)

    SIGMAS.save(0, 0, 0, '', '', outputDir = 'Fits/')
